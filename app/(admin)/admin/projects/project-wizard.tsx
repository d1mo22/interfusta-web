			...
			<div className="max-w-[1280px] mx-auto px-6 lg:px-20 pt-8 pb-16">
				...
				<div key={step} className="motion-rise mt-10">
					...
					<div className="mt-10">
						...
						{step === 1 && (
							<NameStep draft={draft} categories={categories} errors={errors} set={set} />
						)}
						{step === 2 && <DescriptionStep draft={draft} errors={errors} set={set} />}
						{step === 3 && (
							<DetailsStep
								draft={draft}
								errors={errors}
						...
						{step === REVIEW && (
							<ReviewStep draft={draft} photos={photos} category={category} goTo={goTo} />
						)}
					</div>
				</div>
			</div>
...
type Errors = ReturnType<typeof stepErrors>;
...
function ActionBar({
	...
	return (
		// Pinned to the bottom so it's always under the thumb.
